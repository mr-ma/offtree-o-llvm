...
static cl::opt<bool> SplitAll("split_all", cl::init(false), cl::desc("Run basic block splitting in all functions"));
...
bool SplitBasicBlock::runOnFunction(Function &F) {
  ...
  // Do we obfuscate
  if (toObfuscate(SplitAll, tmp, "split")) {
    errs() << "Running SPLIT: " << F.getName() <<"\n";
    split(tmp);
    ++Split;
  }
