...
static cl::opt<bool> SubAll("sub_all", cl::init(false), cl::desc("Substitute all functions"));
...
bool Substitution::runOnFunction(Function &F) {
  ...
  Function *tmp = &F;
  // Do we obfuscate
  if (toObfuscate(SubAll, tmp, "sub")) {
    errs() << "Running SUB: " << F.getName() <<"\n";
    substitute(tmp);
	return true;
  }
