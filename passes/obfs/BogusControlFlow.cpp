...
static cl::opt<bool> BcfAll("bcf_all", cl::init(false), cl::desc("Insert bogus control flow in all functions"));
static cl::opt<int>
...
namespace {
  struct BogusControlFlow : public FunctionPass {
    ...
    virtual bool runOnFunction(Function &F){
      ...
      // If fla annotations
      if(toObfuscate(BcfAll, &F, "bcf")) {
        errs() << "Running BCF: " << F.getName() <<"\n";
        bogus(F);
        doF(*F.getParent());
        return true;
      }
